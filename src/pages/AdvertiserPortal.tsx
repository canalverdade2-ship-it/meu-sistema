...
export function AdvertiserPortal() {
  ...
  if (!snapshot) return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-neutral-950 px-4 py-10 sm:py-16 text-white">
      <div className="w-full max-w-xl overflow-hidden rounded-[2.5rem] border border-white/10 bg-neutral-900/90 p-6 shadow-2xl backdrop-blur-xl sm:p-10">
        ...
        <div className="mb-6 flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-400 text-neutral-950 shadow-lg shadow-amber-400/20"><Megaphone className="h-6 w-6" /></div><div><h1 className="text-xl font-black">Portal do Anunciante</h1><p className="text-xs text-white/50">Acesso conectado ao Supabase</p></div></div>
        <div className="mb-6 grid grid-cols-2 gap-2 rounded-2xl border border-white/5 bg-black/40 p-1.5"><button type="button" onClick={() => setAccessMode('protocol')} className={`rounded-lg py-2 text-xs font-black ${accessMode === 'protocol' ? 'bg-amber-400 text-neutral-950' : 'text-white/60'}`}><KeyRound className="mr-1 inline h-4 w-4" />Primeiro acesso</button><button type="button" onClick={() => setAccessMode('email')} className={`rounded-lg py-2 text-xs font-black ${accessMode === 'email' ? 'bg-amber-400 text-neutral-950' : 'text-white/60'}`}><Mail className="mr-1 inline h-4 w-4" />Entrar por e-mail</button></div>
  ...
  return (
    <main className="min-h-screen bg-neutral-100 text-neutral-900">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="h-fit rounded-2xl bg-neutral-950 p-3 text-white lg:sticky lg:top-22">{tabs.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold ${tab === id ? 'bg-white text-neutral-950' : 'text-white/55 hover:bg-white/5 hover:text-white'}`}><Icon className="h-4 w-4" />{label}</button>)}</aside>
...
function Card({ children }: { key?: string; children: React.ReactNode }) { return <article className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm">{children}</article>; }
function ListEmpty({ empty, text, children }: { empty: boolean; text: string; children: React.ReactNode }) { return <div className="space-y-3">{empty ? <div className="rounded-2xl border border-dashed border-neutral-200 bg-white p-10 text-center text-sm font-bold text-neutral-400">{text}</div> : children}</div>; }
