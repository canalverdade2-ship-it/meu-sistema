...
import {
  ...
  type FreeToolId,
} from './FreeToolsCalculatorDialog';
...
const TOOLS: ToolCard[] = [
  {
    ...
    description: 'Estimativa das principais verbas de encerramento de um vínculo CLT.',
    category: 'Trabalhista',
    useCase: 'Para compreender a composição aproximada da rescisão antes da conferência oficial.',
  ...
  },
  {
    ...
    description: 'Panorama inicial da regra geral e de duas regras de transição consideradas em 2026.',
    category: 'Previdenciário',
    useCase: 'Para comparar idade e contribuição com critérios previdenciários básicos.',
  ...
  },
  {
    ...
    description: 'Estimativa bruta da remuneração de férias e do adicional constitucional de um terço.',
    category: 'Trabalhista',
    useCase: 'Para visualizar o valor bruto antes dos descontos e das condições específicas do vínculo.',
  ...
  },
  {
    ...
    category: 'Previdenciário',
    useCase: 'Para identificar as informações necessárias antes de uma análise completa.',
  ...
  },
  {
    ...
    useCase: 'Para compreender os pontos normalmente avaliados em um pedido.',
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 border-b border-[#d4ccbe] pb-8 lg:grid-cols-[1fr_0.7fr] lg:items-end">
            <div className="max-w-3xl">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#806128]">Ferramentas disponíveis</p>
              <h2 id="free-tools-title" className="mt-3 text-3xl font-black tracking-[-0.035em] text-[#111820] sm:text-5xl">Escolha a consulta que precisa iniciar.</h2>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[#606a73] sm:text-base">Ao abrir uma ferramenta, a calculadora será apresentada em um ambiente próprio, sem tirar você desta página.</p>
            </div>
            <div className="flex items-start gap-3 border-l-2 border-[#c7a458] pl-5 text-sm leading-6 text-[#626c75]">
              <LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-[#8a6e2f]" />
              Nenhum CPF, nome, telefone ou documento é solicitado para realizar os cálculos.
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {AVAILABLE_TOOLS.map(({ id, icon: Icon, number, title, description, category, useCase, includes }) => (
              <button
                ...
                className="group relative flex min-h-[390px] flex-col overflow-hidden rounded-2xl border border-[#d4cdc2] bg-[#fbf9f4] p-6 text-left shadow-[0_12px_32px_rgba(24,32,40,0.055)] transition duration-200 hover:-translate-y-1 hover:border-[#ad8b42] hover:bg-white hover:shadow-[0_22px_48px_rgba(24,32,40,0.11)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9f8140] focus-visible:ring-offset-2 focus-visible:ring-offset-[#eee9df] sm:p-7"
              >
                <span className="absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,#8a6b2f,#d8bd73,transparent)] opacity-80" />

                <div className="flex items-start justify-between gap-4">
                  <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#172433] text-[#d8bd73]"><Icon className="h-5 w-5" /></span>
                  <div className="text-right">
                    <span className="block text-[10px] font-black tracking-[0.2em] text-[#9b7c33]">{number}</span>
                    <span className="mt-2 inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-emerald-700">Disponível</span>
                  </div>
                </div>

                <p className="mt-7 text-[10px] font-black uppercase tracking-[0.17em] text-[#806128]">{category}</p>
                ...
                <div className="mt-6 border-t border-[#ded8ce] pt-5">
                  <p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#858c91]">Indicado para</p>
                ...
                <div className="mt-auto flex items-center justify-between gap-4 border-t border-[#ded8ce] pt-5">
                  <span className="text-[10px] font-bold text-[#838a90]">Consulta educativa</span>
                  <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.09em] text-[#765b25]">Abrir calculadora <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
                </div>
              </button>
            ))}
          </div>
      ...
      <section className="border-y border-[#d7d1c6] bg-[#f8f5ef] py-12 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#806128]">Próximas ferramentas</p>
              <h2 className="mt-3 text-2xl font-black tracking-[-0.03em] text-[#111820] sm:text-3xl">Novas consultas em desenvolvimento.</h2>
            </div>
            <p className="max-w-lg text-sm leading-6 text-[#667078]">Essas opções aparecem somente como previsão da evolução da área e ainda não recebem dados do usuário.</p>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {FUTURE_TOOLS.map(({ id, icon: Icon, number, title, description, category }) => (
              <article key={id} className="rounded-2xl border border-[#d7d1c6] bg-white/55 p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e6e1d8] text-[#737b80]"><Icon className="h-5 w-5" /></span>
                  <span className="text-[10px] font-black tracking-[0.18em] text-[#9a8a68]">{number}</span>
                </div>
                <p className="mt-5 text-[9px] font-black uppercase tracking-[0.15em] text-[#887a5d]">{category}</p>
                <h3 className="mt-2 text-lg font-black text-[#222b32]">{title}</h3>
                <p className="mt-3 text-xs leading-5 text-[#6c747a]">{description}</p>
                <span className="mt-5 inline-flex rounded-full border border-[#d5cec3] bg-[#f2eee7] px-3 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-[#7c8388]">Em desenvolvimento</span>
              </article>
            ))}
          </div>
        </div>
