...
function TerminationPro() {
  ...
  const report: CalculatorPdfReport = {
    ...
    mode: 'pro',
    headline: `Total bruto aproximado: ${currency.format(result.total)}`,
    summary: 'Relatório completo com os dados informados e a memória das parcelas estimadas.',
    sections: [
      { title: 'Dados do vínculo', rows: [
        ...
        { label: 'Avos de 13º', value: `${numeric(thirteenth)} de 12` },
        { label: 'Avos de férias', value: `${numeric(vacation)} de 12` },
        { label: 'Saldo aproximado do FGTS', value: currency.format(numeric(fgts)) },
        { label: 'Possui férias vencidas', value: yesNo(expired) },
      ] },
      { title: 'Memória do cálculo', rows: [
        ...
        { label: 'Multa estimada do FGTS', value: currency.format(result.fgtsPenalty) },
        { label: 'Total bruto aproximado', value: currency.format(result.total) },
      ] },
    ],
    disclaimer: 'Estimativa educativa antes de descontos e particularidades do contrato. O saldo do FGTS não integra o total e o saque depende da modalidade e da situação do trabalhador.',
  };
  return <Workbench title="Memória avançada da rescisão" description="Preencha períodos, FGTS e condições do vínculo para detalhar as parcelas consideradas." result={<Result eyebrow="Total bruto aproximado" headline={currency.format(result.total)} summary="Soma das parcelas estimadas antes de descontos e situações especiais." icon={<Calculator className="h-5 w-5" />} note="O saldo do FGTS não integra o total. O saque depende da modalidade e da situação do trabalhador." report={report}><ResultLine label="Saldo de salário" value={currency.format(result.salaryBalance)} />{result.notice > 0 && <ResultLine label={`Aviso-prévio (${decimal.format(result.noticeDays)} dias)`} value={currency.format(result.notice)} />}<ResultLine label="13º proporcional" value={currency.format(result.thirteenthValue)} /><ResultLine label="Férias proporcionais + 1/3" value={currency.format(result.proportionalVacation)} />{expired && <ResultLine label="Férias vencidas + 1/3" value={currency.format(result.expiredVacationValue)} />}<ResultLine label="Multa estimada do FGTS" value={currency.format(result.fgtsPenalty)} emphasized /></Result>}>
...
function RetirementPro() {
  ...
  const report: CalculatorPdfReport = {
    ...
    mode: 'pro',
    headline: result.anyEligible ? 'Há uma regra possível com os dados informados' : 'Nenhuma das regras comparadas foi atingida',
    summary: `${decimal.format(result.currentAge)} anos de idade, ${decimal.format(result.contributionYears)} anos de contribuição e ${decimal.format(result.points)} pontos.`,
    sections: [
      { title: 'Dados previdenciários', rows: [
        ...
        { label: 'Contribuía antes de 13/11/2019', value: yesNo(beforeReform) },
        { label: 'Pontuação atual', value: `${decimal.format(result.points)} pontos` },
      ] },
      { title: 'Comparação das regras', rows: [
        { label: 'Regra geral', value: `${result.generalEligible ? 'Requisitos atingidos' : 'Requisitos pendentes'} - ${missing(result.currentAge, result.generalAge, 'idade')}; ${missing(result.contributionYears, result.generalContribution, 'contribuição')}` },
        ...(beforeReform ? [
          { label: 'Regra dos pontos', value: `${result.pointsEligible ? 'Requisitos atingidos' : 'Requisitos pendentes'} - ${missing(result.points, result.transitionPoints, 'pontuação')}; ${missing(result.contributionYears, result.transitionContribution, 'contribuição')}` },
          { label: 'Idade mínima progressiva', value: `${result.progressiveEligible ? 'Requisitos atingidos' : 'Requisitos pendentes'} - ${missing(result.currentAge, result.progressiveAge, 'idade')}; ${missing(result.contributionYears, result.transitionContribution, 'contribuição')}` },
        ] : []),
      ] },
    ],
    disclaimer: 'Panorama educativo baseado nos dados digitados. Somente contribuições reconhecidas no CNIS devem ser consideradas. Pedágios, atividade especial, magistério, trabalho rural, deficiência e direito adquirido exigem análise própria.',
  };
  return <Workbench title="Comparação avançada das regras" description="Considere apenas vínculos e contribuições reconhecidos no CNIS." result={<Result eyebrow="Panorama informado" headline={result.anyEligible ? 'Há uma regra possível' : 'Continue acompanhando'} summary={`${decimal.format(result.currentAge)} anos · ${decimal.format(result.contributionYears)} anos de contribuição · ${decimal.format(result.points)} pontos`} icon={<Landmark className="h-5 w-5" />} note="Pedágios, atividade especial, magistério, trabalho rural, deficiência e direito adquirido exigem análise própria." report={report} action={<a href="https://meu.inss.gov.br/" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[#d8bd73]/40 px-4 text-sm font-black text-[#efd991]">Conferir no Meu INSS <ExternalLink className="h-4 w-4" /></a>}><div className="grid gap-3 py-4 xl:grid-cols-3"><Requirement title="Regra geral" eligible={result.generalEligible} lines={[missing(result.currentAge, result.generalAge, 'Idade'), missing(result.contributionYears, result.generalContribution, 'Contribuição')]} />{beforeReform && <Requirement title="Regra dos pontos" eligible={result.pointsEligible} lines={[missing(result.points, result.transitionPoints, 'Pontuação'), missing(result.contributionYears, result.transitionContribution, 'Contribuição')]} />}{beforeReform && <Requirement title="Idade progressiva" eligible={result.progressiveEligible} lines={[missing(result.currentAge, result.progressiveAge, 'Idade'), missing(result.contributionYears, result.transitionContribution, 'Contribuição')]} />}</div></Result>}>
...
function VacationPro() {
  ...
  const report: CalculatorPdfReport = {
    ...
    summary: 'Relatório completo da composição bruta para 30 dias de férias, incluindo médias informadas.',
    sections: [
      { title: 'Dados informados', rows: [
        { label: 'Salário bruto mensal', value: currency.format(numeric(salary)) },
        { label: 'Média de adicionais e variáveis', value: currency.format(numeric(averages)) },
      ] },
      { title: 'Composição do resultado', rows: [
        ...
        { label: 'Total bruto antes dos descontos', value: currency.format(result.total) },
      ] },
      { title: 'Condições consideradas', items: ['Período de 30 dias de férias.', 'Médias informadas somadas à remuneração.', 'Não inclui venda de 10 dias, adiantamento do 13º, faltas ou férias em dobro.'] },
    ],
    disclaimer: 'O valor líquido varia conforme INSS, Imposto de Renda, faltas, médias e condições específicas. Confirme os valores na folha de pagamento.',
  };
  return <Workbench title="Composição avançada das férias" description="Inclua médias habituais de adicionais e valores variáveis." result={<Result eyebrow="Total bruto estimado" headline={currency.format(result.total)} summary="Valor antes dos descontos aplicáveis ao pagamento de férias." icon={<Palmtree className="h-5 w-5" />} note="O valor líquido varia conforme INSS, Imposto de Renda, faltas, médias e condições específicas." report={report}><ResultLine label="Remuneração de férias" value={currency.format(result.remuneration)} /><ResultLine label="Adicional constitucional de 1/3" value={currency.format(result.constitutionalThird)} /><ResultLine label="Total antes dos descontos" value={currency.format(result.total)} emphasized /></Result>}>
