...
export type ProEligibilityResult = 'eligible' | 'ineligible' | 'verification_error';
...
const SOURCE_LABELS: Record<string, string> = {
  client_paid_invoice: 'Benefício de cliente GSA',
  manual: 'Liberação administrativa',
...
export function FreeToolsProEligibilityDialog({
  ...
  const sourceLabel = source ? SOURCE_LABELS[source] || 'Acesso Pro autorizado' : 'Benefício de cliente GSA';
  ...
  return (
    <AccessibleDialog
      isOpen={isOpen}
      onClose={onClose}
      ariaLabel={eligible ? 'Acesso Pro liberado' : verificationError ? 'Verificação do acesso Pro' : 'Acesso Pro não liberado'}
      panelClassName="max-w-[680px] overflow-hidden rounded-2xl border border-[#c8bda9] bg-[#f7f3eb] shadow-[0_40px_120px_rgba(4,12,18,0.58)]"
    ...
    >
      <div className="max-h-[calc(100dvh-1.5rem)] overflow-y-auto sm:max-h-[calc(100dvh-3rem)]">
        <div className={`h-1.5 ${eligible ? 'bg-emerald-600' : verificationError ? 'bg-[#a97625]' : 'bg-[#b77229]'}`} />
        ...
        <header className="flex items-start justify-between gap-4 border-b border-[#ddd5c9] bg-[#fbf8f2] px-5 py-5 sm:px-7 sm:py-6">
          <div className="flex min-w-0 items-start gap-4">
            <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-white ${eligible ? 'bg-emerald-700' : verificationError ? 'bg-[#8a6428]' : 'bg-[#9a5722]'}`}>
              {eligible ? <BadgeCheck className="h-6 w-6" /> : verificationError ? <CircleAlert className="h-6 w-6" /> : <LockKeyhole className="h-6 w-6" />}
            </span>
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#806128]">Retorno do acesso de cliente</p>
              <h2 className="mt-1 text-2xl font-black leading-tight tracking-[-0.03em] text-[#111820]">
                {eligible ? 'Acesso Pro liberado.' : verificationError ? 'Não foi possível concluir a verificação.' : 'O modo Pro permanece bloqueado.'}
              </h2>
          ...
          {eligible ? (
            <>
              <p className="text-base leading-7 text-[#4f5a63]">
                Seu login foi confirmado e a conta atende às condições de liberação automática. A calculadora avançada já está aberta e pronta para uso.
              </p>
              ...
              <div className="mt-6 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50">
                <div className="flex items-start gap-4 border-b border-emerald-200 px-5 py-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-emerald-800">A autorização foi validada no servidor antes da abertura do modo Pro.</p>
                  </div>
                </div>
                <div className="grid gap-px bg-emerald-200 sm:grid-cols-3">
                  {[
                    ['Conta identificada', ShieldCheck],
                    ['Benefício validado', FileCheck2],
                    ['Pro desbloqueado', BadgeCheck],
                  ].map(([label, Icon]) => {
                    const StatusIcon = Icon as typeof ShieldCheck;
                    return (
                      <div key={label as string} className="flex items-center gap-2 bg-white/75 px-4 py-3 text-xs font-black text-emerald-900">
                        <StatusIcon className="h-4 w-4 text-emerald-700" />
                        {label as string}
                      </div>
                    );
                  })}
            ...
            <>
              <p className="text-base leading-7 text-[#4f5a63]">
                O login foi concluído, porém o sistema não conseguiu confirmar a elegibilidade desta conta neste momento. Nenhum acesso foi concedido sem validação.
              </p>
              <div className="mt-6 flex items-start gap-3 rounded-2xl border border-[#ddc99f] bg-[#faf2df] p-5 text-sm leading-6 text-[#665127]">
                ...
                Você pode continuar usando o cálculo Free ou consultar as demais formas de desbloqueio do Pro.
              </div>
            </>
          ) : (
            <>
              <p className="text-base leading-7 text-[#4f5a63]">
                Seu login foi realizado com sucesso, mas a conta não recebeu a liberação automática do modo Pro.
              </p>

              <section className="mt-6 rounded-2xl border border-[#ddd4c7] bg-white p-5 sm:p-6">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#806128]">Condições do benefício automático</p>
                <div className="mt-4 space-y-3">
                  {[
                    'Cadastro do cliente com situação ativa.',
                    'Pelo menos uma fatura registrada como paga.',
                    'Benefício automático habilitado pelo administrador para esta calculadora.',
                  ].map((item) => (
                    <div key={item} className="flex items-start gap-3 text-sm leading-6 text-[#4e5962]">
                      <Check className="mt-1 h-4 w-4 shrink-0 text-[#8a6e2f]" />
                      {item}
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-4 flex items-start gap-3 rounded-xl border border-[#e0c5a8] bg-[#fbf0e4] p-4 text-xs leading-5 text-[#744b29]">
                ...
                O cálculo Free continua disponível. Para usar o Pro agora, também é possível realizar o pagamento avulso ou aplicar um voucher válido.
              </div>
            </>
          )}
        </main>
        ...
        <footer className="flex flex-col-reverse gap-2 border-t border-[#d8d0c4] bg-[#fbf8f2] px-5 py-4 sm:flex-row sm:items-center sm:justify-end sm:px-7">
          ...
          >
            {eligible ? 'Acessar calculadora Pro' : 'Continuar no Free'}
          </button>
