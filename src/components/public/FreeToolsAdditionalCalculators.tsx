...
function requirementRows(requirements: ScreeningRequirement[], detailed: boolean): CalculatorReportRow[] {
  return requirements.map((requirement) => ({
    label: requirement.label,
    value: detailed
      ? `${requirement.met ? 'Atendido' : 'Pendente'} - ${requirement.detail}`
      : requirement.met ? 'Atendido' : 'Pendente',
  }));
}
...
function FreeLayout({ eyebrow, title, description, form, result, proItems, report }: {
  eyebrow: string;
  title: string;
  description: string;
  form: ReactNode;
  ...
  return (
    <div className="grid overflow-hidden rounded-2xl border border-[#d4cdc2] bg-white shadow-[0_20px_55px_rgba(29,36,42,0.09)] lg:grid-cols-[1fr_0.88fr]">
      ...
      <aside className="bg-[#152433] p-5 text-white sm:p-7 lg:p-8">
        ...
        <div className="mt-7 border-t border-white/10 pt-6"><CalculatorPdfReportButton report={report} mode="free" /></div>
...
function Section({ number, title, description, children }: { number: string; title: string; description?: string; children: ReactNode }) {
  return (
    <section className="border-b border-[#e3ddd4] pb-7 last:border-0 last:pb-0">
      <div className="mb-5 flex items-start gap-3"><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#c9b57f] bg-[#f7f0dc] text-[9px] font-black text-[#765b25]">{number}</span><div><h3 className="text-sm font-black text-[#111820]">{title}</h3>{description && <p className="mt-1 text-xs leading-5 text-[#6b747c]">{description}</p>}</div></div>
      {children}
    </section>
  );
}
...
function Workbench({ title, description, children, result }: { title: string; description: string; children: ReactNode; result: ReactNode }) {
  return (
    <div className="grid overflow-hidden rounded-2xl border border-[#d4cdc2] bg-white shadow-[0_24px_65px_rgba(29,36,42,0.11)] lg:grid-cols-[1.06fr_0.94fr]">
      <section className="min-w-0 bg-[#fffdfa]"><div className="border-b border-[#e2dcd2] bg-[#faf7f1] px-5 py-5 sm:px-7"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-[#806128]">Modo Pro · análise avançada</p><h3 className="mt-2 text-xl font-black tracking-[-0.025em] text-[#111820]">{title}</h3><p className="mt-2 text-xs leading-5 text-[#68727a]">{description}</p></div><div className="space-y-7 p-5 sm:p-7 lg:p-8">{children}</div></section>
      {result}
    </div>
  );
}

function ResultPanel({ eyebrow, headline, summary, icon, children, note, report, action }: { eyebrow: string; headline: string; summary: string; icon: ReactNode; children: ReactNode; note: string; report: CalculatorPdfReport; action?: ReactNode }) {
  return (
    <aside className="bg-[#132231] p-5 text-white sm:p-7 lg:sticky lg:top-0 lg:self-start">
      ...
      <div className="mt-6 border-t border-white/10 pt-5"><CalculatorPdfReportButton report={report} mode="pro" /></div>
...
function benefitInputRows(state: ReturnType<typeof useBenefitScreening>): CalculatorReportRow[] {
  const common = [{ label: 'Benefício consultado', value: BENEFIT_LABELS[state.type] }];
  if (state.type === 'temporary_incapacity') return [...common, { label: 'Qualidade de segurado', value: yesNo(state.insured) }, { label: 'Contribuições mensais', value: `${numeric(state.contributions)} mês(es)` }, { label: 'Dias de incapacidade', value: `${numeric(state.incapacityDays)} dia(s)` }, { label: 'Hipótese de isenção de carência', value: yesNo(state.exempt) }, { label: 'Prova médica disponível', value: yesNo(state.medical) }];
  if (state.type === 'maternity') return [...common, { label: 'Qualidade de segurado', value: yesNo(state.insured) }, { label: 'Documento do evento disponível', value: yesNo(state.eventDocumented) }];
  if (state.type === 'death_pension') return [...common, { label: 'Cobertura do segurado falecido', value: yesNo(state.deceasedCoverage) }, { label: 'Dependente previsto em lei', value: yesNo(state.dependent) }, { label: 'Prova de vínculo ou dependência', value: yesNo(state.dependencyEvidence) }];
  return [...common, { label: 'Qualidade de segurado na data do acidente', value: yesNo(state.insured) }, { label: 'Categoria abrangida', value: yesNo(state.eligibleCategory) }, { label: 'Sequela permanente', value: yesNo(state.permanentSequela) }, { label: 'Redução da capacidade', value: yesNo(state.reducedCapacity) }];
}
...
export function ThirteenthPro() {
  ...
  const report: CalculatorPdfReport = {
    calculator: 'Calculadora de 13º salário',
    mode: 'pro',
    headline: `Segunda parcela líquida estimada: ${currency.format(result.secondInstallmentNet)}`,
    summary: `13º bruto de ${currency.format(result.grossValue)} para ${result.eligibleMonths} mês(es) computados.`,
    sections: [
      { title: 'Dados informados', rows: [
        { label: 'Salário bruto mensal', value: currency.format(numeric(salary)) },
        { label: 'Média de comissões e adicionais', value: currency.format(numeric(averages)) },
        { label: 'Meses com 15 dias ou mais', value: `${result.eligibleMonths} de 12` },
        { label: 'Primeira parcela já paga', value: currency.format(numeric(firstPaid)) },
        { label: 'INSS informado', value: currency.format(numeric(inss)) },
        { label: 'Imposto de Renda informado', value: currency.format(numeric(irrf)) },
        { label: 'Outros descontos', value: currency.format(numeric(others)) },
      ] },
      { title: 'Memória completa do cálculo', rows: [
        { label: 'Remuneração considerada', value: currency.format(result.baseRemuneration) },
        { label: '13º bruto proporcional', value: currency.format(result.grossValue) },
        { label: 'Primeira parcela informada', value: currency.format(result.firstInstallmentPaid) },
        { label: 'Segunda parcela antes dos descontos', value: currency.format(result.secondInstallmentBeforeDeductions) },
        { label: 'Total de descontos informados', value: currency.format(result.totalDeductions) },
        { label: 'Segunda parcela líquida estimada', value: currency.format(result.secondInstallmentNet) },
        { label: 'Total líquido estimado recebido', value: currency.format(result.estimatedTotalNet) },
      ] },
    ],
    disclaimer: 'INSS e Imposto de Renda devem ser informados conforme a folha ou cálculo oficial. A ferramenta não substitui o contracheque ou a apuração do empregador.',
  };
  return <Workbench title="Memória avançada do 13º salário" description="Organize remuneração, avos, adiantamento e descontos já apurados no contracheque." result={<ResultPanel eyebrow="Segunda parcela líquida estimada" headline={currency.format(result.secondInstallmentNet)} summary={`13º bruto de ${currency.format(result.grossValue)} para ${result.eligibleMonths} mês(es) computados.`} icon={<HandCoins className="h-5 w-5" />} note="INSS e Imposto de Renda devem ser informados conforme a folha ou cálculo oficial. A ferramenta não substitui o contracheque." report={report}><ResultLine label="Remuneração considerada" value={currency.format(result.baseRemuneration)} /><ResultLine label="13º bruto proporcional" value={currency.format(result.grossValue)} /><ResultLine label="1ª parcela informada" value={currency.format(result.firstInstallmentPaid)} /><ResultLine label="2ª parcela antes dos descontos" value={currency.format(result.secondInstallmentBeforeDeductions)} /><ResultLine label="Descontos informados" value={currency.format(result.totalDeductions)} /><ResultLine label="2ª parcela líquida estimada" value={currency.format(result.secondInstallmentNet)} emphasized /></ResultPanel>}>
...
export function BpcPro() {
  ...
  const report: CalculatorPdfReport = {
    calculator: 'Triagem BPC / LOAS',
    mode: 'pro',
    headline: `${headline} - renda por pessoa de ${currency.format(state.result.incomePerPerson)}`,
    summary: `${currency.format(state.result.familyGrossIncome)} divididos por ${state.result.familyMembers} integrante(s).`,
    sections: [
      { title: 'Dados do requerente e grupo familiar', rows: [
        { label: 'Modalidade', value: state.applicantType === 'elderly' ? 'Pessoa idosa' : 'Pessoa com deficiência' },
        ...(state.applicantType === 'elderly' ? [{ label: 'Idade', value: `${state.result.age} ano(s)` }] : [{ label: 'Impedimento de longo prazo', value: yesNo(state.longTerm) }]),
        { label: 'Renda bruta mensal do grupo', value: currency.format(state.result.familyGrossIncome) },
        { label: 'Quantidade de integrantes', value: `${state.result.familyMembers} pessoa(s)` },
        { label: 'CadÚnico atualizado', value: yesNo(state.cadUnico) },
        { label: 'CPF de todos os integrantes', value: yesNo(state.cpfs) },
        { label: 'Cadastro biométrico disponível', value: yesNo(state.biometric) },
        { label: 'Recebe benefício possivelmente incompatível', value: yesNo(state.otherBenefit) },
      ] },
      { title: 'Referências e resultado', rows: [
        { label: 'Salário mínimo de referência', value: currency.format(MINIMUM_WAGE_2026) },
        { label: 'Limite objetivo por pessoa', value: currency.format(state.result.incomeLimit) },
        { label: 'Renda familiar por pessoa', value: currency.format(state.result.incomePerPerson) },
        { label: 'Situação dos critérios objetivos', value: state.result.allObjectiveCriteriaMet ? 'Todos os critérios informados foram atendidos' : 'Existem critérios informados como pendentes' },
      ] },
      { title: 'Análise detalhada dos critérios', rows: requirementRows(state.result.requirements, true) },
      ...(state.result.missing.length ? [{ title: 'Pendências identificadas', items: state.result.missing.map((requirement) => `${requirement.label}: ${requirement.detail}`) }] : []),
    ],
    disclaimer: 'A vulnerabilidade social, exclusões de renda e demais circunstâncias dependem da análise administrativa. O BPC não paga 13º salário e não gera pensão por morte.',
  };
  return <Workbench title="Triagem completa do BPC / LOAS" description="Compare renda, perfil, Cadastro Único, biometria e impedimentos objetivos do requerimento." result={<ResultPanel eyebrow="Renda familiar por pessoa" headline={currency.format(state.result.incomePerPerson)} summary={`${currency.format(state.result.familyGrossIncome)} divididos por ${state.result.familyMembers} integrante(s).`} icon={<HeartHandshake className="h-5 w-5" />} note="A vulnerabilidade social e as exclusões de renda dependem da análise administrativa. A ferramenta usa o limite objetivo de um quarto do salário mínimo de 2026." report={report} action={<a href="https://meu.inss.gov.br/" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[#d8bd73]/40 px-4 text-sm font-black text-[#efd991]">Solicitar pelo Meu INSS <ExternalLink className="h-4 w-4" /></a>}><ResultLine label="Salário mínimo de referência" value={currency.format(MINIMUM_WAGE_2026)} /><ResultLine label="Limite objetivo por pessoa" value={currency.format(state.result.incomeLimit)} emphasized /><div className="mt-5"><RequirementList requirements={state.result.requirements} /></div><div className="mt-5 rounded-xl border border-white/10 bg-white/[0.04] p-4 text-xs leading-5 text-white/60"><Info className="mr-2 inline h-4 w-4 text-[#d8bd73]" />O BPC tem valor mensal de um salário mínimo, não paga 13º salário e não gera pensão por morte.</div></ResultPanel>}>
