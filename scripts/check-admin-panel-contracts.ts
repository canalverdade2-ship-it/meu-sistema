...
async function assertFileExcludes(path: string, patterns: string[]) {
  const content = await readFile(resolve(root, path), 'utf8');
  for (const pattern of patterns) {
    assert.ok(!content.includes(pattern), `${path}: padrão inseguro presente: ${pattern}`);
  }
}
...
async function main() {
  ...
  console.log('Painel administrativo, colaborador e sessão pública: contratos de segurança validados.');
}
