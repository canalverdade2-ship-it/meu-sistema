...
const migrationFiles = [
  ...
  '20260901160000_gsa_tv_live_control_console.sql',
];
...
const env = [
  ...
  'CHANNEL_ID=ch-main', 'FFPLAYOUT_URL=http://127.0.0.1:8787',
...
work=/tmp/gsa-tv-control-plane-1_6_7
existing_token=$(sudo docker inspect gsa-tv-control-plane --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null | awk -F= '$1=="INTERNAL_API_TOKEN"{sub(/^INTERNAL_API_TOKEN=/,"");print;exit}' || true)
if [ -z "$existing_token" ]; then existing_token=$(openssl rand -hex 32); fi
...
printf '%s' '${payload64}' | base64 -d | python3 -c 'import sys,json,os; d=json.load(sys.stdin); b="/tmp/gsa-tv-control-plane-1_6_7"; [(os.makedirs(os.path.dirname(b+"/"+k),exist_ok=True),open(b+"/"+k,"w",encoding="utf-8").write(v)) for k,v in d.items()]'
printf '%s' '${env64}' | base64 -d > "$work/.env"
printf 'INTERNAL_API_TOKEN=%s\n' "$existing_token" >> "$work/.env"
`;
...
if ! sudo test -s /opt/gsa-tv/cache/media/1/filler/gsa-tv-filler-600.mp4; then
  sudo docker run --rm --user 989:989 -v /opt/gsa-tv/fallback:/fallback:ro -v /opt/gsa-tv/cache/media:/media gsa-tv/control-plane:1.6.7 ffmpeg -hide_banner -loglevel error -stream_loop 19 -i /fallback/gsa-tv-fallback-720p30.mp4 -t 600 -c copy -movflags +faststart /media/1/filler/gsa-tv-filler-600.mp4
fi
