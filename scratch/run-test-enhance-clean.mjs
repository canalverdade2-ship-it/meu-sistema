...
async function run() {
  ...
  console.log('Arquivo original:', sourcePath);
  ...
  const videoFilters = 'scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0x07111f,fps=30,eq=contrast=1.07:brightness=-0.012:saturation=1.12,unsharp=5:5:0.85:5:5:0.0,format=yuv420p';
  ...
  console.log('Processando remasterizacao broadcast...');
  ...
  console.log('Nova Resolucao:', vStream.width + 'x' + vStream.height);
  console.log('Bitrate de Video:', Math.round(Number(vStream.bit_rate || 0) / 1000) + ' kbps');
  console.log('Audio Sample Rate:', aStream?.sample_rate + ' Hz');
  await pool.end();
}
...
sudo docker cp /tmp/test-enhance.js gsa-tv-control-plane:/app/test-enhance.js
sudo docker exec gsa-tv-control-plane node /app/test-enhance.js
sudo docker exec gsa-tv-control-plane rm -f /app/test-enhance.js
rm -f /tmp/test-enhance.js
`;
