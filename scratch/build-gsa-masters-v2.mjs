...
const out=path.join(base,'masters-v2');
...
for(const [slug,opening,closing] of map) for(const [kind,n] of [['opening',opening],['closing',closing]]) {
 ...
 const fc=`[0:v]scale=2304:1296:force_original_aspect_ratio=increase,crop=1920:1080,setpts=1.25*PTS,fps=30,trim=duration=10,split[sharp][blur];[blur]crop=1420:860:250:110,gblur=sigma=42[center];[sharp][center]overlay=250:110,drawbox=x=0:y=0:w=iw:h=ih:color=black@0.13:t=fill[scene];[scene]drawbox=x=250:y=110:w=1420:h=860:color=0x020713@0.78:t=fill:enable='gte(t,${logoStart-0.45})'[base];[1:v]scale=1050:760:force_original_aspect_ratio=decrease,format=rgba,fade=t=in:st=${logoStart}:d=0.65:alpha=1,fade=t=out:st=${logoOut}:d=0.35:alpha=1[lg];[base][lg]overlay=(W-w)/2:(H-h)/2:enable='gte(t,${logoStart})'[v];[0:a]atempo=0.8,atrim=duration=10,afade=t=in:st=0:d=0.35,afade=t=out:st=9.35:d=0.65[a]`;
