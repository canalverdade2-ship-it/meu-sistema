...
const newBuilderPy = `import base64
import fcntl
import hashlib
import json
import os
...
PROFILE = {'width': 1280, 'height': 720, 'fps': 30, 'audio_rate': 48000, 'audio_channels': 2}
...
def safe_media_path(value):
    ...
    roots = [MEDIA_ROOT.resolve(), BASE.resolve()]
...
def cache_key(path, kind, title=''):
    st = path.stat()
    raw = f'{path}|{st.st_mtime_ns}|{st.st_size}|{kind}|{title}|720p30-v1'.encode()
...
def normalize_video(src, trim=None):
    key = cache_key(src, 'video', str(trim or ''))
    out = CACHE / f'{key}.mp4'
...
def render_audio_bumper(audio, program_name, label, job_dir):
    key = cache_key(audio, 'bumper', f'{program_name}|{label}')
    out = CACHE / f'{key}.mp4'
    if out.exists() and out.stat().st_size > 1024:
        return out
    ...
    vf = (
        ...
        "drawbox=x=0:y=0:w=iw:h=110:color=0x1D2A44:t=fill,"
        f"drawtext=fontfile={FONT_BOLD}:text='GSA TV':fontcolor=white:fontsize=38:x=48:y=34,"
        f"drawtext=fontfile={FONT_BOLD}:textfile={program_file}:fontcolor=white:fontsize=64:x=(w-text_w)/2:y=255,"
        f"drawtext=fontfile={FONT_REG}:textfile={label_file}:fontcolor=white@0.90:fontsize=34:x=(w-text_w)/2:y=350,"
        f"fade=t=in:st=0:d=0.25,fade=t=out:st={fade_start:.3f}:d=0.35"
    )
    args = ['-y', '-f', 'lavfi', '-i', f'color=c=0x101826:s=1280x720:r=30:d={dur:.3f}',
...
def prepare_segments(resolved, program, job_dir):
    result = []
    for index, item in enumerate(resolved, start=1):
        src = Path(item['arquivo'])
        if item['tipo'] == 'video':
            out = normalize_video(src, item.get('trim'))
        else:
            out = render_audio_bumper(src, program, item.get('label') or program, job_dir)
...
def build(manifest, force=False):
    ...
    with open(LOCK, 'w') as lockf:
        ...
        try:
            segments = prepare_segments(validation['timeline'], validation['programa'], job_dir)
...
async function main() {
  const b64 = Buffer.from(newBuilderPy, 'utf8').toString('base64');
  const deployScript = `
python3 -c "
import base64
content = base64.b64decode('${b64}').decode('utf-8')
with open('/home/opc/gsa-program-builder/builder.py', 'w', encoding='utf-8') as f:
    f.write(content)
print('Successfully written /home/opc/gsa-program-builder/builder.py')
"
python3 -m py_compile /home/opc/gsa-program-builder/builder.py
echo "PY_COMPILE_STATUS: $?"
`;
  const r = await runSshScript(deployScript);
  console.log('STDOUT:\n' + r.stdout);
  console.log('STDERR:\n' + r.stderr);
}
