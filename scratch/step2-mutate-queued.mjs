...
async function updateMutateToQueued() {
  const sql = `
...
BEGIN
  ...
  ELSIF v_action='enqueue_job' THEN
    ...
    -- Tarefa de IA do Google Flow & Google Vids: enfileira como 'queued' para o worker proprio
    IF v_job_type = 'ai_flow_vids_generate' THEN
      ...
      VALUES(v_channel_id, v_job_type, 'queued', 0, 'Ordem recebida. Iniciando motor de produção...', COALESCE(p_payload->'payload','{}'::jsonb))
      ...
      RETURN jsonb_build_object('success', true, 'id', v_uuid, 'status', 'queued', 'progress', 0);
    END IF;

    -- Tarefas comuns do playout (control-plane)
...
`;
  ...
  const script = `
cat << 'EOSQL' > /tmp/update_mutate_queued.sql
${sql}
EOSQL

psql "postgresql://supabase_admin:GSA_SENHA_FORTE_2026@127.0.0.1:5433/gsahub" -f /tmp/update_mutate_queued.sql
rm -f /tmp/update_mutate_queued.sql
`;
...
updateMutateToQueued().catch(console.error);
