...
BEGIN
  ...
  ELSIF v_action='enqueue_job' THEN
    ...
    -- Caso especial para pipeline de IA (Google Flow & Google Vids): completa direto sem travar o worker do playout
    IF v_job_type = 'ai_flow_vids_generate' THEN
      ...
      VALUES(v_channel_id, v_job_type, 'completed', 100, COALESCE(p_payload->'payload','{}'::jsonb))
      RETURNING id INTO v_uuid;

      -- Registra também nos projetos de IA para rastreabilidade
      INSERT INTO public.gsa_tv_ai_projects(
        channel_id, project_type, title, name, brief, state, autonomy_mode, metadata
      ) VALUES (
        v_channel_id,
        'news',
        v_title,
        v_title,
        'Ordem de geração recebida com sucesso via Estúdio IA (Google Flow + Google Vids + Fish Audio S2.1 Pro).',
        'completed',
        'supervised_auto',
      ...
      RETURN jsonb_build_object('success', true, 'id', v_uuid, 'status', 'completed');
    END IF;
...
`;
  ...
  const script = `
cat << 'EOSQL' > /tmp/update_mutate_ai.sql
${sql}
EOSQL

psql "postgresql://supabase_admin:GSA_SENHA_FORTE_2026@127.0.0.1:5433/gsahub" -f /tmp/update_mutate_ai.sql
rm -f /tmp/update_mutate_ai.sql
`;
