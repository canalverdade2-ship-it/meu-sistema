...
BEGIN
  ...
  ELSIF v_action='enqueue_job' THEN
    ...
    -- Se ja existe job pendente, retorna sucesso
...
\`;
  ...
  try {
    ...
    console.log('SUCCESS: gsa_admin_gsa_tv_mutate updated successfully!');
  } catch (err) {
...
sudo docker cp /tmp/update_mutate.js gsa-tv-control-plane:/tmp/update_mutate.js
sudo docker exec gsa-tv-control-plane node /tmp/update_mutate.js
rm -f /tmp/update_mutate.js
`;

  const res = await runSshScript(bashScript);
  console.log('OUTPUT:\n', res.stdout);
}
