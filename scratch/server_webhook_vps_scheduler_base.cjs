...
function callGSAAssistantOnce(fromPhone, userMessage, session, mediaType, catalog, callback) {
  if (!GEMINI_API_KEY) {
...
6. FORMATAÇÃO LIMPA DE TEXTO (WHATSAPP NATIVO):
   ...
   - Separe frases completas, perguntas, orientações e mudanças de assunto com uma linha em branco (duas quebras de linha: \\n\\n), para a mensagem não ficar visualmente grudada.
   - Mantenha itens consecutivos de menus e listas juntos, com apenas uma quebra de linha entre eles.
   - Não quebre frases no meio e nunca insira espaços extras dentro de valores, números, links ou códigos.
...
function callGSAAssistant(fromPhone, userMessage, session, mediaType, catalog, callback) {
  ...
  const execute = () => {
    attempt += 1;
    callGSAAssistantOnce(fromPhone, userMessage, session, mediaType, catalog, (error, result) => {
      if (error && attempt < maxAttempts) {
        const retryDelay = 500 * attempt;
        console.warn(`Gemini indisponivel na tentativa ${attempt}/${maxAttempts}. Nova tentativa em ${retryDelay}ms: ${error.message}`);
...
function handleProtocolSelfServiceFlow(fromPhone, rawText, session, matchedProtocolCode) {
  ...
  if (matchedProtocolCode) {
    ...
    supabaseGet('/rest/v1/parceiros_resgates?codigo_gerado=eq.' + encodeURIComponent(code) + '&select=*,parceiros(id,name,slug,logo_url,benefits)&limit=1', (err, rows) => {
      ...
      const directCancellationRequest = /\b(cancelar|cancele|cancelamento|desistir|desisto|anular)\b/i.test(text);
      if (directCancellationRequest) {
        ...
        sendWhatsAppReply(
          ...
          [
            ...
            `${partnerName}`,
            ``,
            `Deseja realmente cancelar este resgate?`,
            ``,
            `• Digite *SIM* para confirmar o cancelamento.`,
            `• Digite *NÃO* para manter o benefício ativo.`
          ].join('\n')
        );
        return;
      }
        ...
        if (nlu.intent === 'confirmar_cancelamento') {
          ...
          supabasePatch('/rest/v1/parceiros_resgates?id=eq.' + encodeURIComponent(session.protocolRecord.id), { status: 'cancelado', data_cancelamento: timestamp }, (errPatch, resPatch) => {
            if (errPatch) {
              sendWhatsAppReply(
                fromPhone,
                `Não foi possível concluir o cancelamento neste momento.\n\nPor favor, tente novamente em alguns instantes.`
              );
              return;
            }

            const cancelledProtocolCode = session.protocolCode;
            const cancelledRecord = session.protocolRecord;
            const cancelledPartnerName = cancelledRecord.parceiro_nome;
            ...
            sendWhatsAppReply(
              fromPhone,
              [
                `❌ Protocolo ${cancelledProtocolCode} cancelado com sucesso.`,
                ``,
                `O seu resgate para *${cancelledPartnerName}* foi cancelado em nosso sistema.`,
                ``,
...
function checkDuplicateRedemptionDb(parceiroId, email, telefone, callback) {
  ...
  if (cleanPhone) {
    ...
    if (cleanPhone.startsWith('55') && cleanPhone.length > 10) {
      orFilters.push(`telefone.eq.${cleanPhone.substring(2)}`);
    } else if (!cleanPhone.startsWith('55')) {
      orFilters.push(`telefone.eq.55${cleanPhone}`);
    }
  }

  const url = `/rest/v1/parceiros_resgates?parceiro_id=eq.${parceiroId}&status=not.in.(recusado,cancelado)&or=(${orFilters.join(',')})&select=id,status,codigo_gerado,nome_completo,email,telefone&limit=1`;
  supabaseGet(url, (err, rows) => {
...
function handleRedemptionSuccess(fromPhone, session, partner, form, result, forceOverride) {
  ...
  if (!isDelay24h) {
    ...
    replyLines.push(`🔖 *PROTOCOLO DE ATENDIMENTO:*`);
    replyLines.push(`\`${protocol}\``);
    replyLines.push(``);
    ...
    sendWhatsAppReply(
      ...
      [
        ...
        ``,
        `🎁 *BENEFÍCIO:*`,
        `${partner.benefits || 'Condição exclusiva GSA HUB'}`,
        ``,
        `📌 *Próximos Passos:*`,
  ...
  session.state = 'MAIN_MENU';
  session.lastRedemptionProtocol = protocol;
  session.redemptionPartner = null;
  session.redemptionCandidates = [];
  session.redemptionForm = null;
  session.redemptionDuplicateRecord = null;
    ...
    case 'search_service': {
      ...
      fetchCatalogForAI((catalog) => {
        ...
        if (realService) {
          ...
          item.id = realService.id || item.id || null;
...
function parseServiceRequestMessage(text) {
  ...
  for (let rawLine of lines) {
    ...
    if (!packageName && /(?:pacote)(?!\s+id)\*?:\s*/i.test(cleanLine)) {
      packageName = cleanLine.replace(/^.*?(?:pacote)(?!\s+id)\*?:\s*/i, '').replace(/[\*\_]/g, '').trim();
      continue;
    }

    if (!packageId && /(?:pacote id)\*?:\s*/i.test(cleanLine)) {
      packageId = cleanLine.replace(/^.*?(?:pacote id)\*?:\s*/i, '').replace(/[\*\_]/g, '').trim();
      continue;
    }
...
function handleServiceRequest(fromPhone, text, session) {
  ...
  const orcRand = crypto.randomBytes(4).toString('hex').toUpperCase();
  ...
  const finalizeServiceRequest = (clientObj) => {
    ...
    const serviceContext = session.aiFoundService || null;
    if (serviceContext?.id) orcData.servico_id = serviceContext.id;
    if (parsed.packageId && /^[0-9a-f-]{36}$/i.test(parsed.packageId)) {
      orcData.pacote_servico_id = parsed.packageId;
    }
    ...
    if (clientId) {
      supabasePost('/rest/v1/orcamentos', orcData, (errOrc, resOrc) => {
        if (errOrc) {
          ...
          sendWhatsAppReply(fromPhone, 'Não foi possível registrar sua solicitação neste momento.\n\nNenhum protocolo foi confirmado. Por favor, tente novamente em alguns instantes.');
          return;
        }
        const created = (resOrc && resOrc[0]) ? resOrc[0] : null;
        if (!created?.id) {
          sendWhatsAppReply(fromPhone, 'Não foi possível confirmar a gravação da solicitação.\n\nPor favor, tente novamente.');
          return;
        }
        sendResponse(created);
        notifyAdmin(`🛠️ *NOVA SOLICITAÇÃO DE SERVIÇO*\n\n📋 *Protocolo:* ${created.codigo_orcamento}\n👤 *Cliente:* ${rawNome}\n📞 *WhatsApp:* ${fromPhone}\n📦 *Pacote/Serviço:* ${parsed.packageName}`);
      });
    } else {
      ...
      supabasePost('/rest/v1/clientes', tempClientData, (errC, resC) => {
        if (errC || !resC || !resC[0]?.id) {
          console.error('❌ Erro ao criar lead para solicitação de serviço:', errC);
          sendWhatsAppReply(fromPhone, 'Não foi possível identificar ou cadastrar seus dados para esta solicitação.\n\nNenhum protocolo foi gerado. Por favor, tente novamente.');
          return;
        }
        ...
        supabasePost('/rest/v1/orcamentos', orcData, (errOrc, resOrc) => {
          if (errOrc || !resOrc || !resOrc[0]?.id) {
            console.error('❌ Erro ao criar orçamento para novo lead:', errOrc);
            sendWhatsAppReply(fromPhone, 'Seu contato foi identificado, mas não foi possível registrar a solicitação.\n\nNenhum protocolo foi confirmado. Por favor, tente novamente.');
            return;
          }
          const created = resOrc[0];
          sendResponse(created);
          notifyAdmin(`🛠️ *NOVA SOLICITAÇÃO DE SERVIÇO*\n\n📋 *Protocolo:* ${created.codigo_orcamento}\n👤 *Cliente:* ${tempClientData.nome}\n📞 *WhatsApp:* ${fromPhone}\n📦 *Pacote/Serviço:* ${parsed.packageName}`);
        });
      });
    }
  };
...
// Recupera o LID original quando a Evolution normaliza o webhook para o
// numero convencional. Sem o LID, o WhatsApp aceita o envio, mas pode
// rejeitar a entrega depois com o codigo 463.
function fetchEvolutionMessageLid(messageId) {
  return new Promise((resolve) => {
    if (!messageId) return resolve(null);

    const payload = JSON.stringify({ where: { key: { id: messageId } } });
    const request = http.request({
      hostname: '127.0.0.1',
      port: 8080,
      path: '/chat/findMessages/GSA_WhatsApp',
      method: 'POST',
      headers: {
        apikey: process.env.EVOLUTION_API_KEY || 'gsa_hub_evolution_token_2026',
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(payload)
      }
    }, (response) => {
      ...
      response.on('end', () => {
        try {
          ...
          const originalJid = records[0]?.key?.remoteJid || '';
          resolve(originalJid.includes('@lid') ? originalJid : null);
        } catch (_) {
          resolve(null);
        }
      });
...
async function processMessage(fromPhone, textBody, mediaType, pushName, rawMessageData = {}) {
  ...
  const isRedemptionCancellation = /\b(cancelar|cancele|cancelamento|desistir|desisto|anular)\b[\s\S]*\b(resgate|benef[ií]cio|pedido|solicita[cç][aã]o|protocolo)\b|\b(resgate|benef[ií]cio|pedido|solicita[cç][aã]o|protocolo)\b[\s\S]*\b(cancelar|cancele|cancelamento|desistir|desisto|anular)\b/i.test(text);
  if (isRedemptionCancellation) {
    if (session.lastRedemptionProtocol) {
      handleProtocolSelfServiceFlow(fromPhone, text, session, session.lastRedemptionProtocol);
    } else {
      const localPhone = stripCountryCode55(fromPhone);
      const phoneWithCountryCode = `55${localPhone}`;
      const latestProtocolPath = `/rest/v1/parceiros_resgates?or=(telefone.eq.${encodeURIComponent(localPhone)},telefone.eq.${encodeURIComponent(phoneWithCountryCode)})&select=codigo_gerado,created_at&order=created_at.desc&limit=1`;

      supabaseGet(latestProtocolPath, (err, rows) => {
        const latestProtocol = !err && Array.isArray(rows) && rows[0] && rows[0].codigo_gerado
          ? rows[0].codigo_gerado
          : null;

        if (latestProtocol) {
          session.lastRedemptionProtocol = latestProtocol;
          userSessions[fromPhone] = session;
          handleProtocolSelfServiceFlow(fromPhone, text, session, latestProtocol);
          return;
        }

        sendWhatsAppReply(
          fromPhone,
          `Não encontrei um protocolo de resgate vinculado a este WhatsApp.\n\nEnvie o *PROTOCOLO DE ATENDIMENTO* para que eu possa localizar a solicitação.`
        );
      });
    }
    return;
  ...
  const isProtocolInquiry = /\b(qual|cad[eê]|consultar|consulta|ver|informar|informe|numero|número|codigo|código|status|andamento)\b[\s\S]*\bprotocolo\b|\bprotocolo\b[\s\S]*\b(qual|cad[eê]|consultar|consulta|ver|informar|informe|numero|número|codigo|código|status|andamento)\b/i.test(text);
  if (isProtocolInquiry) {
    if (session.lastRedemptionProtocol) {
      sendWhatsAppReply(
        fromPhone,
        [
          `🔖 *PROTOCOLO DE ATENDIMENTO:*`,
          `\`${session.lastRedemptionProtocol}\``,
          ``,
          `_Guarde este código para acompanhar a sua solicitação._`
        ].join('\n')
      );
    } else {
      const localPhone = stripCountryCode55(fromPhone);
        ...
        if (latestProtocol) {
          ...
          sendWhatsAppReply(
            ...
            [
              ...
              `\`${latestProtocol}\``,
              ``,
              `_Este é o protocolo mais recente vinculado ao seu WhatsApp._`
            ].join('\n')
          );
          return;
        }
  ...
  if (session.state === 'SERVICE_REQUEST_FOLLOWUP') {
    ...
    if (text === '3') {
      ...
      portalMsg += `👉 https://grupo-gsa.com.br/login\n\n`;
  ...
  if (session.state === 'SERVICE_REQUEST_COLLECT_DETAILS') {
    ...
    if (!orcId) {
      sendWhatsAppReply(fromPhone, 'Não foi possível localizar o registro desta solicitação.\n\nEnvie novamente o número do protocolo ou fale com nosso atendimento.');
      return;
    }

    supabaseGet(`/rest/v1/orcamentos?id=eq.${encodeURIComponent(orcId)}&select=observacoes_servico&limit=1`, (errGet, rows) => {
      if (errGet || !Array.isArray(rows) || !rows[0]) {
        sendWhatsAppReply(fromPhone, 'Não foi possível carregar a solicitação para incluir os detalhes.\n\nTente novamente em alguns instantes.');
        return;
      }

      const previousNotes = String(rows[0].observacoes_servico || '').trim();
      const mergedNotes = previousNotes
        ? `${previousNotes}\n\n${noteData.observacoes_servico}`
        : noteData.observacoes_servico;

      supabasePatch(`/rest/v1/orcamentos?id=eq.${encodeURIComponent(orcId)}`, { observacoes_servico: mergedNotes }, (errPatch, updatedRows) => {
        if (errPatch || !Array.isArray(updatedRows) || !updatedRows[0]) {
          sendWhatsAppReply(fromPhone, 'Não foi possível salvar os detalhes enviados.\n\nNenhuma confirmação foi registrada. Por favor, tente novamente.');
          return;
        }

        session.state = 'MAIN_MENU';
        userSessions[fromPhone] = session;
        notifyAdmin(`📝 *NOVOS DETALHES EM SOLICITAÇÃO DE SERVIÇO*\n\n📋 *Protocolo:* ${protocol}\n📞 *WhatsApp:* ${fromPhone}\n\n${text}`);

        let successMsg = `✅ *Informações registradas com sucesso!*\n\n`;
        successMsg += `Os detalhes enviados foram vinculados ao seu protocolo *${protocol}* e nossa equipe especializada foi notificada.\n\n`;
  ...
  if (session.state === 'SERVICE_INTEREST') {
    ...
    if (text === '1') {
      const serviceRequestText = [
        '#SOLICITACAO_SERVICO_GSA',
        'Olá! Gostaria de solicitar atendimento para serviços GSA HUB:',
        '',
        `📦 *Pacote:* Contratação individual`,
        `🛠️ *Serviços Solicitados:*`,
        `• ${item.nome || 'Serviço solicitado pelo cliente'}`
      ].join('\n');
      handleServiceRequest(fromPhone, serviceRequestText, session);
    } else if (text === '2') {
      ...
      notifyAdmin(`💬 *CLIENTE SOLICITOU CONSULTOR DE SERVIÇOS*\n\n📞 *WhatsApp:* ${fromPhone}\n🛠️ *Serviço:* ${item.nome || 'Não especificado'}`);
    } else {
...
async function handleSupabaseWebhook(req, res, bodyData) {
  try {
    ...
    const clientPhone = getClientPhone(record);
    const clientName = getDisplayName(record);
        ...
        if (type === 'INSERT') {
          ...
          if (oldStatus !== newStatus) {
            ...
            if (clientPhone) {
              ...
              if (msgStatus) {
                sendWhatsAppReply(clientPhone, `Olá, ${clientName}!\n\n${msgStatus}\n\n*Pedido:* #${record.id}`);
              }
            }
          }
        }
      ...
      req.on('end', async () => {
        ...
        try {
          ...
          if (data.data && data.data.key) {
            ...
            if (
              addressingMode === 'lid'
              && !actualTargetJid.includes('@lid')
              && data.data.key.id
            ) {
              const recoveredLid = await fetchEvolutionMessageLid(data.data.key.id);
              if (recoveredLid) {
                actualTargetJid = recoveredLid;
                console.log(`LID recuperado para ${fromPhone}: ${recoveredLid}`);
              } else {
                console.warn(`Nao foi possivel recuperar o LID para ${fromPhone}`);
              }
            }
