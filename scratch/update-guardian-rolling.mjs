...
def check_encoder_health(state):
    ...
    if current_seq is not None:
        ...
        # Keep last 5 samples (~2.5 minutes)
        history = [h for h in history if (now - h[0]) <= 180.0]
        state["hls_history"] = history

        # Calculate FPS using the oldest sample in the window (at least 30s apart)
        candidates = [h for h in history if (now - h[0]) >= 25.0]
        if candidates:
...
sudo rm -f /var/run/gsa-process-guardian-state.json
echo "Updated guardian deployed."
`;
