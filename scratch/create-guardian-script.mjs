...
const script = `
set -e
echo "=== WRITING /opt/gsa-tv/bin/gsa-process-guardian.sh ==="

cat << 'EOF' | sudo tee /opt/gsa-tv/bin/gsa-process-guardian.sh > /dev/null
#!/usr/bin/env python3
import sys
import os
import time
...
def log(msg, level="INFO"):
    ...
    try:
        ...
        sys.stderr.write(f"Failed to write to {LOG_FILE}: {e}\n")
...
def check_encoder_health(state):
    ...
    try:
        req = urllib.request.Request(
            ENCODER_STATUS_URL,
            headers={"Authorization": f"Bearer {ENCODER_TOKEN}"}
        )
        ...
        log(f"[ENCODER HEALTH] API unreachable: {e}", "ALERT")
    ...
    try:
        ...
        log(f"[ENCODER HEALTH] Failed to read HLS playlist: {e}", "WARN")
    ...
    if current_seq is not None and prev_seq is not None and prev_time is not None:
        ...
        if dt >= 5.0 and dseq >= 0:
            # 2.0s per segment * 30 fps = 60 frames per segment
            fps = (dseq * 60.0) / dt
            if fps < 29.5:
                log(f"[ENCODER HEALTH] Low FPS detected: {fps:.2f} fps (< 29.5) over {dt:.1f}s ({dseq} segments, age: {hls_age}s)", "ALERT")
            else:
                log(f"[ENCODER HEALTH] Stream FPS healthy: {fps:.2f} fps ({dseq} segments in {dt:.1f}s, hls_age: {hls_age}s)", "INFO")
    elif current_seq is not None:
        log(f"[ENCODER HEALTH] Baseline initialized at HLS sequence #{current_seq} (hls_fresh={hls_fresh}, hls_age={hls_age}s)", "INFO")
...
def get_monitored_processes():
    ...
    try:
        ...
        log(f"Error querying process tree: {e}", "WARN")
...
def check_and_guard_processes(state):
    ...
    for proc in candidates:
        ...
        if cpu >= 80.0:
            ...
            if pid_str not in tracking:
                ...
                log(f"[WATCH] High CPU detected: PID {pid} ({proc['comm']} on CPU {proc['psr']}) at {cpu}% CPU. Tracking initiated.", "WARN")
            else:
                duration = now - tracking[pid_str]
                log(f"[WATCH] PID {pid} ({proc['comm']} on CPU {proc['psr']}) sustained high CPU: {cpu}% for {duration:.0f}s", "WARN")
                if duration >= 180.0:
                    log(f"[ACTION] PID {pid} ({proc['comm']} on CPU {proc['psr']}) exceeded 80% CPU for >3m ({duration:.0f}s). Terminating gracefully (SIGTERM)...", "ALERT")
                    try:
                        ...
                        log(f"[ACTION] PID {pid} terminated successfully.", "INFO")
                    except Exception as e:
    ...
    for tracked_pid in list(tracking.keys()):
        if tracked_pid not in current_high_pids:
            log(f"[WATCH] PID {tracked_pid} normalized below 80% CPU or finished. Removed from tracking.", "INFO")
...
echo "Guardian script created and chmod +x set."
`;
