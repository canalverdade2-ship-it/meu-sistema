...
async function main() {
  const sql = `
-- Permite ai_flow_vids_generate na funcao gsa_admin_gsa_tv_mutate
...
BEGIN
  ...
  ELSIF v_action='enqueue_job' THEN
    ...
    -- Se ja existe job pendente, retorna sucesso com id existente
...
`;
  ...
  const script = `
cat << 'EOSQL' > /tmp/allow_ai_job.sql
${sql}
EOSQL

psql "postgresql://supabase_admin:GSA_SENHA_FORTE_2026@127.0.0.1:5433/gsahub" -f /tmp/allow_ai_job.sql
rm -f /tmp/allow_ai_job.sql
`;
