...
async function runTtsBatch(manifestPath) {
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  if (!Array.isArray(manifest)) throw new Error('Manifesto TTS deve ser uma lista.');
  for (let i = 0; i < manifest.length; i++) {
    const row = manifest[i] || {};
    const text = String(row.text || '').trim();
    const voiceKey = String(row.voice || 'ancora_masc');
    const voiceId = OFFICIAL_VOICES[voiceKey];
    const outputPath = path.resolve(String(row.output || ''));
    if (!text) throw new Error(`Segmento ${i + 1} sem texto.`);
    if (!voiceId) throw new Error(`Segmento ${i + 1}: voz oficial inválida.`);
    if (!outputPath.startsWith('/home/opc/gsa-ai/editions/') && !outputPath.startsWith('/opt/gsa-tv/cache/media/1/')) {
      throw new Error(`Segmento ${i + 1}: destino fora das áreas autorizadas.`);
    }
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });
    await generateTtsAudio(text, voiceId, outputPath);
    console.log(`TTS_BATCH_OK ${i + 1}/${manifest.length} ${voiceKey} ${outputPath}`);
  }
  await pool.end().catch(() => {});
}

if (process.argv[2] === '--tts-batch') {
  const manifestPath = process.argv[3];
  if (!manifestPath) throw new Error('Uso: node ai_worker.mjs --tts-batch <manifest.json>');
  runTtsBatch(manifestPath).catch((error) => { console.error('TTS_BATCH_ERROR', error.message); process.exit(1); });
} else {
