...
(async () => {
  ...
  await flowPage.screenshot({ path: "/tmp/flow_check_ready.png" });
  await browser.disconnect();
})().catch(console.error);
'
base64 -w 0 /tmp/flow_check_ready.png
`;
  const res = await runSshScript(script);
  const buf = Buffer.from(res.stdout.trim().split('\n').pop(), 'base64');
  fs.writeFileSync('public/cast/flow_check_ready.png', buf);
  console.log('Saved flow_check_ready.png:', buf.length, 'bytes');
}
