...
const handleBulkDelete = async () => {
    ...
    const toastId = toast.loading(`Excluindo ${ids.length} produto(s)...`);
    try {
      const idsSet = new Set(ids);
      setSelectedIds(new Set());
      setProdutos((prev) => prev.filter((p) => !idsSet.has(p.id)));

      await deleteAdminProductsBulk(ids);
      toast.success(`${ids.length} produto(s) excluído(s) com sucesso.`, { id: toastId });
      await logService.logAction({
          ...
          >
            ...
            {produto.controle_estoque && (
              <span className={`inline-block mt-1 rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                ...
                ðŸ“¦ Estoque: {produto.estoque_disponivel || 0}
              </span>
            )}
                                    ...
                                    <>
                                      <div className="flex justify-between text-xs">
                                        ...
                                        <span className={`font-bold ${esgotada ? 'text-red-600' : 'text-indigo-600'}`}>
                                          {esgotada ? 'ðŸ”´ Cota esgotada' : `${restante} restantes`}
                                        </span>
                                      </div>
                                      ...
                                      >
                                        ðŸ”„ Iniciar nova campanha e zerar contagem
                                      </button>
                                    </>
                                  );
                                })()}
