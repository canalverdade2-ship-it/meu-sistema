...
for b in blocks:
    ...
    issues = [f"{m['id']}: {media_issue(m, b, date)}" for m in rows]
    print(f"Block {b['name']} ({b['id']}): existing media_item_id={b['media_item_id']}, candidate rows={len(rows)}, issues={issues}")
EOF
