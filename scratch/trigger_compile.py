...
def query(sql, params=None):
    proc = subprocess.run(['node', '-e', PG], input=json.dumps({'sql':sql, 'params':params or []}).encode('utf-8'), capture_output=True, env={'DATABASE_URL':'postgresql://postgres:password@127.0.0.1:5432/gsatv'})
