...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    const err = document.querySelector('[jsslot] [jsname="B1UkEb"]')?.innerText || '';
    return {
      pwValLen: pw ? pw.value.length : -1,
      pwDisabled: pw ? pw.disabled : null,
      buttons,
      err,
  ...
  console.log('Password page state:\\n', JSON.stringify(info, null, 2));
  await b.disconnect();
