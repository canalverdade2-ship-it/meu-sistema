...
def check_encoder_health(state):
    ...
    if current_seq is not None and prev_seq is not None and prev_time is not None:
        ...
        if dt >= 5.0 and dseq >= 0:
            ...
            fps = (dseq * 60.0) / dt
            if fps < 29.5:
                log("[ENCODER HEALTH] Low FPS detected: " + f"{fps:.2f}" + " fps (< 29.5) over " + f"{dt:.1f}" + "s (" + str(dseq) + " segments, age: " + str(hls_age) + "s)", "ALERT")
            else:
                log("[ENCODER HEALTH] Stream FPS healthy: " + f"{fps:.2f}" + " fps (" + str(dseq) + " segments in " + f"{dt:.1f}" + "s, hls_age: " + str(hls_age) + "s)", "INFO")
...
const b64 = Buffer.from(guardianScript, 'utf8').toString('base64');
const remoteDeploy = `
...
python3 -m py_compile /opt/gsa-tv/bin/gsa-process-guardian.sh
echo "Python syntax verification: OK"
`;

const res = await runSshScript(remoteDeploy);
