...
work=/tmp/gsa-tv-control-plane-1_4
...
printf '%s' '${payload64}' | base64 -d | python3 -c 'import sys,json,os; d=json.load(sys.stdin); b="/tmp/gsa-tv-control-plane-1_4"; [(os.makedirs(os.path.dirname(b+"/"+k),exist_ok=True),open(b+"/"+k,"w",encoding="utf-8").write(v)) for k,v in d.items()]'
...
if ! sudo test -s /opt/gsa-tv/cache/media/1/filler/gsa-tv-filler-600.mp4; then
  sudo docker run --rm --user 989:989 -v /opt/gsa-tv/fallback:/fallback:ro -v /opt/gsa-tv/cache/media:/media gsa-tv/control-plane:1.6.4 ffmpeg -hide_banner -loglevel error -stream_loop 19 -i /fallback/gsa-tv-fallback-720p30.mp4 -t 600 -c copy -movflags +faststart /media/1/filler/gsa-tv-filler-600.mp4
fi
