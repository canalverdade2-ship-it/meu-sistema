...
async function main() {
  console.log('=== ATUALIZANDO AS DUAS OPÇÕES COM PRONÚNCIA EXATA: GE.S.A (GÊ-ESSE-Á) ===');

  const mp3Path = 'public/cast/test_gesa.mp3';
  const base64Audio = fs.readFileSync(mp3Path).toString('base64');

  const script = `
VDIR="/opt/gsa-tv/cache/media/1/identity/vinhetas"
echo "${base64Audio}" | base64 -d > "$VDIR/vinheta_ta_na_rede_gesa.mp3"

# Converte para WAV 48kHz estéreo
sudo docker exec gsa-tv-ffplayout ffmpeg -y -hide_banner -loglevel error \
  -i /media/1/identity/vinhetas/vinheta_ta_na_rede_gesa.mp3 \
  -ar 48000 -ac 2 /media/1/identity/vinhetas/vinheta_ta_na_rede_gesa.wav

DUR=$(sudo docker exec gsa-tv-ffplayout ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 /media/1/identity/vinhetas/vinheta_ta_na_rede_gesa.wav)
echo "Nova duração do áudio GE.S.A: $DUR s"

AUDIO_VOX="/media/1/identity/vinhetas/vinheta_ta_na_rede_gesa.wav"
...
sudo docker exec gsa-tv-ffplayout ffmpeg -y -hide_banner -loglevel error \
  ...
  -f lavfi -i "anoisesrc=d=8.1:c=pink:r=48000:a=0.08,lowpass=f=600,afade=t=in:st=0:d=1.5,afade=t=out:st=7.0:d=1.0" \
  -filter_complex \
...
sudo docker exec gsa-tv-ffplayout ffmpeg -y -hide_banner \
  ...
  "[0:v]scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,fps=30, \
   zoompan=z='min(zoom+0.0006,1.14)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=243:s=1920x1080:fps=30, \
   eq=contrast=1.05:saturation=1.08:brightness=0.01, \
   fade=t=in:st=0:d=0.8:color=black, \
   fade=t=out:st=7.3:d=0.8:color=black[outv1]" \
  -map "[outv1]" -map 1:a:0 \
...
echo "Opcao 1 atualizada com pronúncia GE.S.A!"
...
sudo docker exec gsa-tv-ffplayout ffmpeg -y -hide_banner -loglevel error \
  ...
  -f lavfi -i "anoisesrc=d=8.1:c=pink:r=48000:a=0.15,bandpass=f=2000:w=1200,afade=t=in:st=0:d=0.6,afade=t=out:st=0.8:d=0.4" \
  -filter_complex \
...
sudo docker exec gsa-tv-ffplayout ffmpeg -y -hide_banner \
  ...
  -filter_complex \
  "[0:v]scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,fps=30, \
   zoompan=z='if(lte(on,18), 0.45+0.60*(on/18), 1.02+0.02*sin(on*0.18))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=243:s=1920x1080:fps=30, \
   drawbox=x=0:y=0:w=1920:h=1080:color=white@0.85:t=fill:enable='between(t,0.55,0.70)', \
   fade=t=out:st=7.4:d=0.7:color=white[outv2]" \
  -map "[outv2]" -map 1:a:0 \
  -c:v libx264 -preset fast -crf 17 -pix_fmt yuv420p \
  -c:a aac -b:a 256k -ar 48000 -ac 2 \
  -t 8.1 \
