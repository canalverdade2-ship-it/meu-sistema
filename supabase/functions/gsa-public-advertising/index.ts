...
export function normalizePayload(input: unknown) {
  ...
  const payload = {
    ...
    desired_formats: normalizeArray(source.desired_formats),
    desired_pages: normalizeArray(source.desired_pages),
    ...
    notes: String(source.notes || '').trim().slice(0, 4000),
    website_confirmation: String(source.website_confirmation || '').trim(),
    started_at: String(source.started_at || '').trim(),
    source_metadata: typeof source.source_metadata === 'object' && source.source_metadata ? source.source_metadata : {},
  };

  const validSizes = new Set(['autonomo', 'mei', 'micro', 'pequena', 'media', 'grande']);
  const emailValid = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(payload.contact_email);
  const dateRangeValid = !payload.desired_start_date || !payload.desired_end_date || payload.desired_end_date >= payload.desired_start_date;
  const startedAt = Date.parse(payload.started_at);
  const formAgeValid = Number.isFinite(startedAt) && Date.now() - startedAt >= 2500;

  if (
    payload.website_confirmation ||
    payload.company_name.length < 2 ||
    ![11, 14].includes(payload.document.length) ||
    !validSizes.has(payload.company_size) ||
...
export async function handleRequest(request: Request) {
  ...
  if (contentLength > 32_000) return json(413, { error: 'payload_too_large' }, origin);
