...
-- Correção pós-implantação: o gatilho de auditoria é executado depois da
-- alteração. Ao suspender o próprio colaborador, gsa_admin_context() já deve
-- negar o acesso; por isso a auditoria valida diretamente a sessão vinculada
-- ao JWT e registra a alteração sem depender do status novo do colaborador.
