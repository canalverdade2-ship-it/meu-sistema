...
-- 1. Atualiza a função pública de resgate para sempre gerar Protocolo Oficial PROT-RES-YYYY-XXXXXX
...
BEGIN
  ...
  -- Gera o Protocolo de Resgate Oficial Único no formato PROT-RES-2026-XXXXXX
...
-- 2. Atualiza os registros existentes sem código_gerado para terem seu protocolo
UPDATE public.parceiros_resgates
   SET codigo_gerado = 'PROT-RES-' || to_char(created_at, 'YYYY') || '-' || upper(substr(id::text, 1, 6))
 WHERE codigo_gerado IS NULL OR trim(codigo_gerado) = '';
