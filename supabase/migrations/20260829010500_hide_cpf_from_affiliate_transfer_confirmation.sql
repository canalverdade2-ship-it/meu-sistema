...
BEGIN
  ...
  WHERE cliente_id = v_actor.cliente_id AND status = 'ativo' LIMIT 1;
  IF v_sender.id IS NULL THEN RAISE EXCEPTION 'Perfil de afiliado ativo nao encontrado.'; END IF;
  IF length(v_identifier) < 3 THEN RAISE EXCEPTION 'Informe codigo, email, telefone ou CPF do afiliado.'; END IF;
