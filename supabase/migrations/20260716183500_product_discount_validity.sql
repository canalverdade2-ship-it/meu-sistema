-- 20260716183500_product_discount_validity.sql
-- Adiciona suporte a prazo de validade (determinado/indeterminado) nos descontos de produtos

-- 1. Adicionar colunas nas tabelas public.produtos e public.loja_pedido_itens
ALTER TABLE public.produtos
    ADD COLUMN IF NOT EXISTS desconto_prazo_tipo text NOT NULL DEFAULT 'indeterminado',
    ADD COLUMN IF NOT EXISTS desconto_fim_em timestamptz NULL;

ALTER TABLE public.produtos
    DROP CONSTRAINT IF EXISTS check_desconto_prazo_tipo;

ALTER TABLE public.produtos
    ADD CONSTRAINT check_desconto_prazo_tipo
    CHECK (desconto_prazo_tipo IN ('determinado', 'indeterminado'));

ALTER TABLE public.loja_pedido_itens
    ADD COLUMN IF NOT EXISTS desconto_prazo_tipo text NULL,
    ADD COLUMN IF NOT EXISTS desconto_fim_em timestamptz NULL;

-- 2. Sobrescrever gsa_calculate_product_effective_price com novos parâmetros padrão
CREATE OR REPLACE FUNCTION public.gsa_calculate_product_effective_price(
  ...
  p_desconto_prazo_tipo text DEFAULT 'indeterminado',
  p_desconto_fim_em timestamptz DEFAULT NULL
)
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  ...
  -- Verificar prazo de validade determinado (com fuso horário nativo)
  IF p_desconto_prazo_tipo = 'determinado' AND p_desconto_fim_em IS NOT NULL AND now() >= p_desconto_fim_em THEN
    RETURN round(p_valor, 2);
  END IF;
...
-- 3. Atualizar a trigger de cálculo automático de descontos
  ...
  IF NEW.desconto_ativo IS TRUE THEN
    ...
    IF NEW.desconto_prazo_tipo = 'determinado' THEN
      IF NEW.desconto_fim_em IS NULL THEN
        RAISE EXCEPTION 'A data final da promoção deve ser informada.';
      END IF;
      
      -- Permitir salvar no passado apenas se a data de encerramento não estiver mudando (evitar travar outros updates de registros vencidos)
      IF NEW.desconto_fim_em < now() AND (TG_OP = 'INSERT' OR OLD.desconto_fim_em IS DISTINCT FROM NEW.desconto_fim_em) THEN
        RAISE EXCEPTION 'A data final da promoção não pode estar no passado.';
      END IF;
    ELSE
      -- Prazo indeterminado
...
BEFORE INSERT OR UPDATE OF valor, desconto_ativo, desconto_tipo, desconto_valor, desconto_prazo_tipo, desconto_fim_em ON public.produtos
FOR EACH ROW
EXECUTE FUNCTION public.gsa_trg_produtos_discount_calc();

-- 4. Atualizar a RPC gsa_admin_set_product_discount
CREATE OR REPLACE FUNCTION public.gsa_admin_set_product_discount(
  ...
  p_tipo text,
  p_valor numeric,
  p_prazo_tipo text DEFAULT 'indeterminado',
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'desconto_prazo_tipo', v_product.desconto_prazo_tipo,
    'desconto_fim_em', v_product.desconto_fim_em,
...
REVOKE EXECUTE ON FUNCTION public.gsa_admin_set_product_discount(uuid, text, uuid, boolean, text, numeric, text, timestamptz) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.gsa_admin_set_product_discount(uuid, text, uuid, boolean, text, numeric, text, timestamptz) TO authenticated, service_role;

-- 5. Atualizar gsa_client_checkout_store para suportar datas de encerramento do desconto
...
DECLARE
  ...
  -- Novas variáveis de controle para Desconto Individual de Produtos
...
BEGIN
  ...
  LOOP
    IF v_item ->> 'tipo' = 'produto' THEN
      ...
      -- Calcular preços com Desconto Individual considerando o prazo
      v_regular_price := v_product.valor;
      v_effective_price := public.gsa_calculate_product_effective_price(
        v_product.valor,
        v_product.desconto_ativo,
        v_product.desconto_tipo,
        v_product.desconto_valor,
        v_product.desconto_prazo_tipo,
        v_product.desconto_fim_em
      );
      ...
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'desconto_produto_percentual', coalesce(v_product.desconto_percentual, 0.00),
        'desconto_produto_tipo', v_product.desconto_tipo,
        'desconto_produto_configurado', v_product.desconto_valor,
        'desconto_prazo_tipo', v_product.desconto_prazo_tipo,
        'desconto_fim_em', v_product.desconto_fim_em
      ));
  ...
  LOOP
    ...
    -- Buscar elegibilidade considerando o preço promocional
    ...
    IF v_promo.tipo_promocao = 'ganhe_outro_produto' THEN
      ...
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'valor_original', round(v_product.valor, 2),
        'desconto_produto_unitario', 0.00,
        'desconto_produto_percentual', 0.00,
        'desconto_produto_tipo', null,
        'desconto_produto_configurado', null,
        'desconto_prazo_tipo', null,
        'desconto_fim_em', null
      ));
      v_discount_value := round(v_product.valor * v_gift_qty, 2);
    ELSE
      -- Buscar item mais barato considerando o valor unitário promocional (valor_unitario)
  ...
  -- 7. Criar orçamento registrando subtotal_preco_tabela e desconto_produtos
  INSERT INTO public.orcamentos(
    ...
    -- Novas colunas históricas
    subtotal_preco_tabela, desconto_produtos
  )
  SELECT
    ...
    -- Gravar totais de tabela e desconto individual acumulados
    v_subtotal_preco_tabela, v_desconto_produtos
  RETURNING id INTO v_orcamento_id;

  -- 8. Inserir itens do pedido gravando o snapshot histórico do desconto
  INSERT INTO public.loja_pedido_itens(
    ...
    desconto_prazo_tipo, desconto_fim_em
  )
  SELECT
    ...
    -- Preencher com valores históricos do payload do loop do carrinho
    coalesce((entry ->> 'valor_original')::numeric, (entry ->> 'valor_unitario')::numeric),
    ...
    entry ->> 'desconto_prazo_tipo',
    (entry ->> 'desconto_fim_em')::timestamptz
  FROM jsonb_array_elements(v_items) AS x(entry);
  ...
  INSERT INTO public.orcamento_timeline(
    ...
    'Pedido confirmed pelo cliente na GSA Store.',
...
REVOKE EXECUTE ON FUNCTION public.gsa_client_checkout_store(uuid, text, jsonb) FROM public, anon, authenticated;
