  ...
  IF to_regprocedure('public.gsa_admin_write_audit(text,text,text,uuid,jsonb)') IS NULL THEN
    EXECUTE $function$
      ...
      DECLARE
        ...
        v_actor_type text := COALESCE(v_claims -> 'app_metadata' ->> 'gsa_actor_type', 'sistema');
        v_actor_id_text text := COALESCE(v_claims -> 'app_metadata' ->> 'gsa_actor_id', '00000000-0000-0000-0000-000000000000');
        v_id uuid;
      BEGIN
