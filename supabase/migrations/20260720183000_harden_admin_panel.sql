...
DECLARE
  ...
  v_result jsonb;
BEGIN
  SELECT COALESCE(jsonb_agg(limited.item ORDER BY limited.created_at DESC), '[]'::jsonb)
    INTO v_result
    FROM (
      SELECT visible.item, visible.created_at
        FROM (
          ...
          SELECT jsonb_build_object(
            ...
            'created_at', COALESCE((to_jsonb(n) ->> 'data_criacao')::timestamptz, now()),
            'lida', state.read_at IS NOT NULL
          ) AS item,
          COALESCE((to_jsonb(n) ->> 'data_criacao')::timestamptz, now()) AS created_at
          ...
          WHERE state.dismissed_at IS NULL
            AND (
              COALESCE(to_jsonb(n) ->> 'destinatario_tipo', 'admin') IN ('admin', 'broadcast_todos')
              OR (
                v_actor_type = 'colaborador'
                AND to_jsonb(n) ->> 'destinatario_tipo' = 'colaborador'
                AND to_jsonb(n) ->> 'colaborador_id' = v_actor_id::text
              )
            )
        ) visible
...
BEGIN
  FOR v_item IN
    SELECT value
      FROM jsonb_array_elements(public.gsa_admin_list_notifications(p_sessao_id, p_session_token, 100))
  LOOP
...
REVOKE ALL ON FUNCTION public.gsa_admin_search_clients(uuid, text, text, integer) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.gsa_admin_search_clients(uuid, text, text, integer) TO authenticated;
