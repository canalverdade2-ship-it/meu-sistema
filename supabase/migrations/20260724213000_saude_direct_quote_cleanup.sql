...
CREATE TABLE IF NOT EXISTS public.gsa_catalogo_legado_arquivo (
  ...
  CONSTRAINT gsa_catalogo_legado_arquivo_dominio_valido CHECK (dominio IN ('saude', 'seguros')),
  CONSTRAINT gsa_catalogo_legado_arquivo_registro_unico UNIQUE (tabela_origem, registro_hash)
);

ALTER TABLE public.gsa_catalogo_legado_arquivo ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.gsa_catalogo_legado_arquivo FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT ON public.gsa_catalogo_legado_arquivo TO service_role;

ALTER TABLE IF EXISTS public.saude_cotacoes
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_saude_cotacoes_idempotencia
  ON public.saude_cotacoes(cliente_id, idempotency_key)
...
BEGIN
  FOR v_constraint IN
    SELECT conrelid::regclass AS table_name, conname
      FROM pg_constraint
     WHERE contype = 'c'
       AND conrelid = to_regclass('public.saude_cotacoes')
  ...
  IF to_regclass('public.saude_cotacoes') IS NOT NULL THEN
    ALTER TABLE public.saude_cotacoes
      ADD CONSTRAINT saude_cotacoes_categoria_valida
      CHECK (categoria IN ('individual-familiar', 'empresarial', 'odontologico')) NOT VALID;
  END IF;
END;
$constraints$;
  ...
  FOR v_function IN
    SELECT p.oid::regprocedure AS signature
     ...
     WHERE n.nspname = 'public'
       AND p.proname = 'gsa_client_saude_criar_cotacao'
  LOOP
...
DECLARE
  v_cliente_id uuid;
  v_categoria text := replace(lower(trim(COALESCE(p_payload ->> 'categoria', ''))), '_', '-');
  v_localidade text := trim(COALESCE(p_payload ->> 'localidade', ''));
  v_inicio date := nullif(p_payload ->> 'inicio_desejado', '')::date;
  v_idades text := trim(COALESCE(p_payload ->> 'idades', ''));
  v_id uuid;
  v_protocolo text;
BEGIN
  SELECT s.ator_id
  ...
  IF v_categoria NOT IN ('individual-familiar', 'empresarial', 'odontologico') THEN
    RAISE EXCEPTION 'Tipo de plano de saúde inválido.' USING ERRCODE = '22023';
  END IF;

  IF length(v_localidade) < 3 OR v_inicio IS NULL OR length(v_idades) < 1 THEN
    RAISE EXCEPTION 'Localidade, data de início e idades dos beneficiários são obrigatórias.' USING ERRCODE = '22023';
  END IF;
  ...
  v_protocolo := 'SAU-' || to_char(clock_timestamp(), 'YYYYMMDD') || '-' || upper(substr(encode(gen_random_bytes(6), 'hex'), 1, 10));
...
REVOKE ALL ON FUNCTION public.gsa_client_saude_criar_cotacao(uuid, text, jsonb, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.gsa_client_saude_criar_cotacao(uuid, text, jsonb, uuid) TO authenticated, service_role;
...
BEGIN
  ...
  IF v_kind <> 'parceiro' THEN
    RAISE EXCEPTION 'O cadastro de produtos, planos e seguros foi descontinuado.' USING ERRCODE = '22023';
  END IF;
...
BEGIN
  FOREACH v_relation IN ARRAY ARRAY[
    'saude_planos_regioes',
    'saude_planos_precos',
    'saude_produto_regioes',
    'saude_produto_precos',
    'saude_produtos',
    'saude_planos',
    'saude_operadoras',
    'saude_regioes',
    'saude_faixas_etarias',
    'saude_redes_credenciadas',
    'saude_rede_credenciada'
  ]
  LOOP
    IF to_regclass(format('public.%I', v_relation)) IS NOT NULL THEN
      EXECUTE format(
        'INSERT INTO public.gsa_catalogo_legado_arquivo (dominio, tabela_origem, registro_hash, dados)
         SELECT ''saude'', %L, md5(to_jsonb(t)::text), to_jsonb(t)
           FROM public.%I t
         ON CONFLICT (tabela_origem, registro_hash) DO NOTHING',
        v_relation,
        v_relation
      );
    END IF;
...
ALTER TABLE IF EXISTS public.saude_cotacoes DROP COLUMN IF EXISTS produto_id CASCADE;
ALTER TABLE IF EXISTS public.saude_cotacoes DROP COLUMN IF EXISTS plano_id CASCADE;
ALTER TABLE IF EXISTS public.saude_cotacoes DROP COLUMN IF EXISTS oferta_id CASCADE;
ALTER TABLE IF EXISTS public.saude_propostas DROP COLUMN IF EXISTS produto_id CASCADE;
ALTER TABLE IF EXISTS public.saude_propostas DROP COLUMN IF EXISTS plano_id CASCADE;
ALTER TABLE IF EXISTS public.saude_propostas DROP COLUMN IF EXISTS oferta_id CASCADE;
ALTER TABLE IF EXISTS public.saude_contratos DROP COLUMN IF EXISTS produto_id CASCADE;
ALTER TABLE IF EXISTS public.saude_contratos DROP COLUMN IF EXISTS plano_id CASCADE;
ALTER TABLE IF EXISTS public.saude_contratos DROP COLUMN IF EXISTS oferta_id CASCADE;
