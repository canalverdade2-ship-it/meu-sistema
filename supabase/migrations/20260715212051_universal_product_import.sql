-- Table for origin tracking
CREATE TABLE public.produto_importacao_origem (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    produto_id uuid NOT NULL REFERENCES public.produtos(id) ON DELETE CASCADE,
    source_type text NOT NULL CHECK (source_type IN ('url', 'excel', 'pdf', 'txt', 'image')),
    source_fingerprint text NOT NULL,
    source_reference text NULL,
    original_filename text NULL,
    imported_at timestamptz NOT NULL DEFAULT now(),
    imported_by_type text NULL,
    imported_by_id uuid NULL,
    imported_by_name text NULL
);

-- RLS
ALTER TABLE public.produto_importacao_origem ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.produto_importacao_origem FROM public, anon;
GRANT ALL ON TABLE public.produto_importacao_origem TO authenticated, service_role;

-- Bucket for temporary import files
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
    'gsa-product-import-files',
    'gsa-product-import-files',
    false,
    52428800, -- 50 MB
    ARRAY['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel', 'text/csv', 'text/plain', 'text/tab-separated-values', 'image/jpeg', 'image/png', 'image/webp']
)
ON CONFLICT (id) DO UPDATE SET 
    file_size_limit = 52428800,
    allowed_mime_types = ARRAY['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel', 'text/csv', 'text/plain', 'text/tab-separated-values', 'image/jpeg', 'image/png', 'image/webp'];
...
REVOKE EXECUTE ON FUNCTION public.gsa_admin_import_products_batch_v2(uuid, text, jsonb) FROM public, anon;
