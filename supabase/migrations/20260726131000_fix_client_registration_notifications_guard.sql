-- Allow client and supplier onboarding notifications in client notification guard function.
-- Problem: When submitting business client registration (gsa_public_register_client),
-- the notification trigger raised "Sessão de cliente inválida para criar notificação."
-- because client onboarding actions were checked against client session constraints when destinatario_tipo='cliente'.
...
DECLARE
  ...
  v_allowed_admin_actions CONSTANT TEXT[] := ARRAY[
    ...
    'cadastro_cliente', 'bonus_indicacao_cadastro', 'cadastro_prestador',
    'cadastro_fornecedor', 'cadastro_fornecedor_reenviado', 'perfil_fornecedor_atualizado'
  ];
BEGIN
  -- Permite notificações do sistema/onboarding (para admin ou cliente) geradas por fluxos públicos e RPCs
  IF COALESCE(NEW.acao_origem, '') IN (
    'cadastro_cliente', 'bonus_indicacao_cadastro', 'cadastro_fornecedor',
    'cadastro_fornecedor_reenviado', 'cadastro_prestador', 'orcamento_criado',
    'perfil_fornecedor_atualizado'
  ) THEN
