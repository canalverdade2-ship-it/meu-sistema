...
(async () => {
  ...
  console.log("Testing gemini.generateText with gemini-flash-latest...");
  const res = await gemini.generateText({
    apiKey,
    model: "gemini-flash-latest",
    prompt: "Escreva um roteiro curto de teste com 50 palavras sobre culinaria brasileira. Responda em JSON com title e text.",
