...
(async () => {
  const r = await p.query(\`
    ...
    where (b.metadata->>'content_mode' = 'library' or b.is_reprise = true or p.name in ('GSA Em Fé', 'GSA Desenhos', 'GSA Sessão Pipoca', 'GSA Music', 'Continuidade GSA TV'))
      and v.broadcast_date >= '2026-09-13'
    order by v.broadcast_date desc, v.version desc, b.planned_start_offset_s
  \`);
  r.rows.forEach(row => {
    const d = new Date(row.broadcast_date).toISOString().slice(0,10);
    console.log(\`[\${d} v\${row.version} \${row.state}] \${row.program_name} (\${row.planned_duration_s}s) -> block_id: \${row.block_id} | media_id: \${row.media_item_id} | title: \${row.media_title} | dur: \${row.media_dur}s | path: \${row.drive_path}\`);
  });
  await p.end();
