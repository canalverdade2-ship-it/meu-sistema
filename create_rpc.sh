cat << 'EOF' > /home/opc/create_rpc.sql
DROP FUNCTION IF EXISTS public.gsa_admin_delete_products_by_filter(text,text,text,text);
DROP FUNCTION IF EXISTS public.gsa_admin_delete_products_by_filter(uuid,text,text,text,text,text);

CREATE OR REPLACE FUNCTION public.gsa_admin_delete_products_by_filter(
  p_sessao_id uuid,
  p_session_token text,
  p_status text DEFAULT NULL,
  p_tipo_cliente text DEFAULT NULL,
  p_categoria_id text DEFAULT NULL,
  p_search text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_sql text;
  v_ids uuid[];
  v_result jsonb;
BEGIN
  v_sql := 'SELECT array_agg(id) FROM public.produtos WHERE 1=1';

  IF p_status = 'ativo' OR p_status = 'inativo' THEN
    v_sql := v_sql || ' AND status = ''' || p_status || '''';
  END IF;

  IF p_tipo_cliente = 'pf' OR p_tipo_cliente = 'pj' THEN
    v_sql := v_sql || ' AND tipo_cliente IN (''' || p_tipo_cliente || ''', ''ambos'')';
  ELSIF p_tipo_cliente = 'ambos' THEN
    v_sql := v_sql || ' AND tipo_cliente = ''ambos''';
  END IF;

  IF p_categoria_id IS NOT NULL AND p_categoria_id != 'todos' THEN
    IF p_categoria_id = 'sem_categoria' THEN
      v_sql := v_sql || ' AND categoria_id IS NULL';
    ELSE
      v_sql := v_sql || ' AND categoria_id = ''' || p_categoria_id || '''';
    END IF;
  END IF;

  IF p_search IS NOT NULL AND trim(p_search) != '' THEN
    v_sql := v_sql || ' AND (nome ILIKE ''%' || trim(p_search) || '%'' OR codigo_produto ILIKE ''%' || trim(p_search) || '%'')';
  END IF;

  EXECUTE v_sql INTO v_ids;
  
  IF v_ids IS NULL OR array_length(v_ids, 1) = 0 THEN
    RETURN jsonb_build_object('success', true, 'deleted', 0, 'inactivated', 0, 'total', 0);
  END IF;
  
  -- Pass the IDs to the existing safe deletion function
  v_result := public.gsa_admin_delete_products_bulk(p_sessao_id, p_session_token, v_ids);
  
  RETURN v_result;
END;
$$;
EOF
PGPASSWORD='GSA_SENHA_FORTE_2026' psql -h 127.0.0.1 -p 5433 -U supabase_admin -d gsahub -f /home/opc/create_rpc.sql