...
docker exec gsa-tv-ffplayout sqlite3 /state/ffplayout.db ".backup '/backups/ffplayout-${STAMP}.db'"
cp "$BASE/backups/ffplayout/ffplayout-${STAMP}.db" "$TMP/ffplayout.db"
tar -C "$BASE" -czf "$TMP/media-and-playout.tgz" cache/media playlists fallback config/ffplayout
find "$BASE/cache/media" -type f -print0 | sort -z | xargs -0 sha256sum > "$TMP/media.sha256" || true
...
( cd "$TMP" && sha256sum database-full.dump database-gsa-tv.dump ffplayout.db media-and-playout.tgz runtime-config.tgz secrets.tar.enc > manifest.sha256 )
pg_restore --list "$TMP/database-full.dump" >/dev/null
sqlite_tmp="$TMP/ffplayout-restore-test.db"
cp "$TMP/ffplayout.db" "$sqlite_tmp"
docker run --rm -v "$TMP:/restore" alpine:3.22 sh -lc "apk add --no-cache sqlite >/dev/null && sqlite3 /restore/ffplayout-restore-test.db 'pragma integrity_check;'" | grep -qx ok
